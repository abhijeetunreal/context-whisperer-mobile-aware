...
export const useTextToSpeech = ({ voiceId = '9BWtsMINqrJLrRacOk9x', apiKey }: TextToSpeechOptions) => {
  ...
  const generateNaturalDescription = useCallback((currentContext: string, currentObjects: string[], reasoning: string): string => {
    const now = Date.now();
  ...
  const speak = useCallback(async (text: string, currentObjects?: string[], reasoning?: string) => {
    ...
    if (currentObjects && reasoning !== undefined) {
      processedText = generateNaturalDescription(text, currentObjects, reasoning);
    }
